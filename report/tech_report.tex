...
The final model ranked 12th in the round. A minimal PCT already cleared the threshold; the meaningful gains came from the hierarchical sample-and-group stages and the augmentation set, while an experiment that added geometric (pairwise-distance) attention biases underperformed and was dropped. On the main track the emphasis inverted: weeks went into architecture and schedule variants while the noise model and the evaluation harness were fixed late.
