...
\begin{abstract}
This report documents an attempt to reproduce StraightPCF (CVPR 2024), a point-cloud filtering method, in Jittor for Track~2 of the 6th Jittor AI Challenge. The final system is a coupled velocity module of roughly 0.7M parameters that predicts per-point displacements returning noisy samples to the underlying surface, trained on ${\sim}20$k ShapeNet meshes. On a held-out split of the training data it reduces the Chamfer distance by about 51\% and the point-to-surface error by about 67\% relative to the noisy input, giving a best competition score of 67.44/100 and a national top-100 placement. It does not reproduce the paper's reported accuracy. Four causes account for most of the gap: a merged training schedule that trained the distance module differently from the paper's prescription, noise-model drift away from the interpolation family the method assumes, inference-time repetition that compensated for a mediocre velocity field, and a score-based detour that consumed the final weeks. The implementation itself --- chunked KNN, GPU farthest-point sampling, scatter-mean edge convolutions, MPI data parallelism, and an evaluation harness --- is described in the middle sections.
\end{abstract}
...
Jittor accumulates the computation graph until it is executed and collected, and our first implementation fought both out-of-memory failures and intermittent segfaults. The crash signatures were misleading: one run died with a segfault after roughly 2{,}200 iterations, and a later failure read as out-of-memory although the device reported 45.9\,GB of 47\,GB in use with roughly 11{,}000 live operators and 12{,}000 live variables --- the memory was held by the graph, not by the batch. The dominant cause was lazy graph accumulation: a single fusion kernel would hold several hundred k-NN and edge-convolution operators at once. The fix that made long runs possible was an explicit hygiene discipline: \texttt{jt.sync\_all()} (execute and release) followed by \texttt{jt.gc()} after every encoder block, after every training step, and at epoch boundaries; a cap of 8 patches per inference batch; and detaching patch outputs from the graph before blending. The same discipline reduced the peak memory of an encoder forward pass enough to train at batch size 24 with 8 patches per sample. The syncs cost throughput, but the discipline turned a training run that reliably crashed into one that completed 100 epochs unattended.

\subsection{Vectorizing the reconstruction}

The patch-blending reconstruction was initially written as per-patch Python loops, and because every comparison against the assignment mask crossed the GPU$\to$CPU boundary via \texttt{.item()}, a single cloud's reconstruction cost roughly $3\times10^5$ synchronizations. Rewriting the assignment with vectorized \texttt{jt.equal} + \texttt{jt.nonzero} masked scatter brought that to roughly $3\times10^2$. The same pattern --- find the Python loop that syncs per iteration, replace it with a masked tensor operation --- recurred in FPS and in KNN.

\subsection{Distributed training}

Data-parallel training uses Jittor's built-in MPI integration (\texttt{mpirun -np 6 python run.py \dots}): parameters are broadcast at start via \texttt{mpi\_param\_broadcast}, validation losses are all-reduced, and checkpointing/logging are rank-0 only. Two practical notes: dataloaders are constructed before the CUDA context is initialized to avoid the fork-after-CUDA hazard, and the launch configuration carries \texttt{NCCL\_P2P\_DISABLE=1} as a workaround for peer-to-peer collective hangs on the shared node. Epoch time varied by configuration: 340--430\,s for the submission run, about 936\,s for the July reruns of the same coupled configuration, 1{,}400--1{,}900\,s for the distance-module phase --- its Euler chain adds four no-gradient encoder passes per step against two for the velocity modules alone --- and about 813\,s for Stage~1, which supervises all 1{,}000 points per patch.
...
Table~\ref{tab:glance} gives the headline numbers for the best scored submission. The best score was 67.44/100, with the Chamfer half (51.9) noticeably weaker than the P2S half (82.9). Pooled over samples, CD falls from $2.46\times10^{-4}$ to $1.21\times10^{-4}$ (a 51\% reduction) and P2S from $1.96\times10^{-4}$ to $6.4\times10^{-5}$ (a 67\% reduction). The per-sample P2S sub-score (82.9) is higher than the pooled reduction suggests because the score average rewards samples where the prediction beats the noise by a wide margin while clamping the few bad samples at zero.
...
Two observations from Figure~\ref{fig:curves} shaped our reading of the whole effort. The staged pretraining runs are reproducible: VM1 and VM2, trained independently with the same recipe from different seeds, follow nearly identical trajectories (3.78 $\to$ 0.101 over 15 epochs), so the training machinery itself is stable. Both the pretraining and the coupled run also reach their loss floors quickly --- within 5 and about 40 epochs respectively --- after which hundreds of additional epochs changed neither the loss nor the metric. We treated the plateau as an optimization problem for too long. The loss is a proxy for the metric, and Section~\ref{sec:postmortem} argues the binding constraint was the training target rather than the optimizer. The staged rerun that followed stopped before producing a usable model: run alone on the competition's noise distribution, a pretrained single velocity module scores 1.01/100 --- essentially no improvement over the noisy input (CD sub-score 0.00, P2S 2.03).

\subsection{What changed the numbers}

The changes we can attribute improvements to, ordered by confidence: (i)~cutting 8 patches of 1{,}000 points per sample instead of one (an $8\times$ increase in supervised points per sample); (ii)~moving inference from whole-cloud Euler integration to patch-based filtering, which fixed both a train/inference statistics mismatch and the memory ceiling; (iii)~feeding the encoder the same interpolation state that training uses when computing the coupled state ($X_{t_1}$), removing a small distribution shift; (iv)~removing the distance module from the phase-1 forward pass (encoder calls per step 3$\to$2); and (v)~the reconstruction vectorization and sync discipline of Sections~\ref{sec:jittor}. Items (i)--(iii) were validated by the competition metric; (iv)--(v) are throughput and stability wins.
...
\paragraph{The merged schedule and the distance module.} The paper's staged schedule is not incidental: the distance module is trained last \emph{with the backbone frozen}, so that it learns to scale step sizes against a stationary feature distribution. Our competition schedule instead trained the distance module briefly and then unfroze the backbone, after which the features it had learned against changed. The July rerun quantifies the damage: with the backbone unfrozen and the distance module along for the ride, the P2S sub-score fell from 82.9 to 77.9 while CD was unchanged. This matches a step-size regulator that no longer stops points on the surface, with the direction of motion (which CD is more sensitive to) unaffected. It is our best explanation for the gap to the paper, and we found it after the submission window had closed.

\paragraph{Noise-model drift.} The method's mathematics assumes interpolation between a noisy and a clean sample of the \emph{same} noise family: the velocity target $X_1 - X_0$ is meaningful because $X_t$ moves along that line. The competition's noise is Laplace. We trained with a ``mixed'' schedule (60\% Laplace, 20\% Gaussian, 20\% uniform) intended to hedge, which quietly violates the interpolation assumption for two-fifths of the samples. The hedge bought nothing measurable and cost fidelity to the method's premise; the staged reruns use pure Gaussian per the paper. The 1.01/100 measurement of Section~\ref{sec:results} makes the cost concrete: a velocity module trained only on Gaussian noise does not transfer to Laplace noise at all. We should have matched the evaluation noise distribution exactly rather than hedging inside a framework whose math assumes a specific family.

\paragraph{Inference repetition.} Our inference repeats the patch filter 2--4 times depending on estimated noise. This raises the score --- iterated Euclidean projections onto an imperfectly straight field behave like a curved path --- but the reported quality then partly reflects inference compute rather than the velocity field. Under the paper's single pass we could not reproduce the paper's numbers.
...
AdamW (lr $0.01$, weight decay $10^{-4}$), warmup-cosine schedule (10-epoch warmup, $\eta_{\min} = 10^{-6}$), batch size 32, 300 epochs, label smoothing 0.1, dropout 0.5, and weight EMA. Augmentations: random scaling $[0.67, 1.5]$, translation $\pm 0.2$, point dropout 10--40\% (with resampling to preserve the point count), and Gaussian jitter ($\sigma = 0.01$, clipped). Rotation augmentation was present in earlier versions and removed during tuning; that is the only augmentation change with code evidence behind it. Validation used a fixed 90/10 split.

The version history is short. A minimal PCT --- four attention layers, no sample-and-group stages, rotation-only augmentation, SGD with cosine annealing --- already cleared the threshold at roughly 80\% accuracy. An experiment that added a geometric attention bias (a learnable rescaling of pairwise Euclidean distances) together with LayerNorm and a feed-forward block underperformed and was dropped. The gains that mattered came from the hierarchical sample-and-group stages and the widened augmentation set. Per-version accuracies were not retained; the 12th-place finish is the only measured result that survives.

\subsection{A Jittor gotcha worth recording}

Jittor's dropout layers did not follow the module's training flag reliably in our version. The final training script works around this by walking the module tree and setting every dropout layer explicitly (\texttt{\_set\_dropout\_train}) at the start of each epoch and before validation. Until that fix, runs trained with dropout in an unintended state. This is a silent regularization change, not a crash, which is what makes it expensive: nothing in the loss curves or the logs announces it.

The main track drew the opposite lesson from this round. Here, the data-side work --- augmentation ranges, the sample-and-group stages, the validation split --- produced the gains, and architecture variants did not. On the main track, weeks went into architecture and schedule variants while the noise model and the evaluation harness were fixed late.
